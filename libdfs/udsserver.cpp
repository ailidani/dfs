...
void UDSServer::add(UDSSession *session)
{
    sessionLock.lock();
...
void UDSServer::remove(UDSSession *session)
{
    sessionLock.lock();
...
bool UDSServer::hasExt(const string &ext)
{
...
string UDSServer::callExt(const string &ext, const string &data)
{
    return extensions[ext](repo, data);
}

void UDSServer::registerExt(const string &ext, UDSExtCB cb)
{
    extensions[ext] = cb;
}
...
void UDSSession::printError(const std::string &what)
{
    fdwstream fs(fd);
