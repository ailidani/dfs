...
int LocalRepo_Init(const string &rootPath, bool bareRepo, const string &uuid)
{
    string oriPath;
    string tmpDir;
    string objDir;
...
int LocalRepo_PeerHelper(LocalRepo *l, const string &path)
{
    Peer p = Peer(path);
...
void LocalRepo::open(const string &root)
{
    if (root.compare("") != 0) {
        rootPath = root;
    }
...
LocalRepoLock::sp LocalRepo::lock()
{
    ASSERT(opened);
    if (rootPath == "")
 ...
 * Remote Operations
 */
void LocalRepo::setRemote(Repo *r)
{
...
void LocalRepo::setRemoteFlags(bool cacheLocally)
{
    Monitor lock(remoteLock);
...
void LocalRepo::createObjDirs(const ObjectHash &objId)
{
...
string LocalRepo::objIdToPath(const ObjectHash &objId)
{
...
int LocalRepo::addObject(ObjectType type, const ObjectHash &hash,
 ...
 */
ObjectHash LocalRepo::addTree(const Tree &tree)
{
 ...
 */
ObjectHash LocalRepo::addCommit(/* const */ Commit &commit)
{
 ...
 */
std::string LocalRepo::getPayload(const ObjectHash &objId)
{
 ...
 */
size_t LocalRepo::getObjectLength(const ObjectHash &objId)
{
    if (!hasObject(objId)) {
 ...
 * Get the object type.
 */
ObjectType LocalRepo::getObjectType(const ObjectHash &objId)
{
    if (!hasObject(objId)) {
        LOG("Couldn't get object %s", objId.hex().c_str());
 ...
 * Verify object
 */
string LocalRepo::verifyObject(const ObjectHash &objId)
{
    LocalObject::sp o;
    ObjectType type;
 ...
 */
bool LocalRepo::copyObject(const ObjectHash &objId, const string &path)
{
    Object::sp o = getObject(objId);
...
set<ObjectInfo> LocalRepo::listObjects()
{
...
void rebuildIndexCb(const ObjectInfo &info, offset_t off, void *arg)
{
...
void LocalRepo::dumpIndex()
{
    index.dump();
}

void packfileDumper(const ObjectInfo &info, offset_t off, void *arg)
{
    info.print();
    printf("  packfile: offset = 0x%x\n", off);
}

void LocalRepo::dumpPackfile(packid_t id)
{
...
vector<Commit> LocalRepo::listCommits()
{
    vector<Commit> rval;
...
map<string, ObjectHash> LocalRepo::listSnapshots()
{
    return snapshots.getList();
}

ObjectHash LocalRepo::lookupSnapshot(const string &name)
{
...
struct MultiPullOp {
    ...
    // Pull queue
    deque<ObjectHash> toPull;
    std::unordered_set<ObjectHash> toPullSet;
                   ...
                   const std::string &dstPath)
{
	...
	std::unordered_set<ObjectHash> parents = gDag.getParents(*it);
	std::unordered_set<ObjectHash>::iterator p;
...
set<string> LocalRepo::listBranches()
{
 ...
 */
void LocalRepo::setBranch(const std::string &name)
{
    // Verify branch name
 ...
 */
void LocalRepo::updateHead(const ObjectHash &commitId)
{
    string branch = getBranch();
    string headPath;
 ...
 */
void LocalRepo::setHead(const ObjectHash &commitId)
{
    string ref = "#" + commitId.hex();
    OriFile_WriteFile(ref.c_str(), ref.size(), rootPath + ORI_PATH_HEAD);
}
 ...
 * Set merge state
 */
void LocalRepo::setMergeState(const MergeState &state)
{
 ...
 * Get merge state
 */
MergeState LocalRepo::getMergeState()
{
    string mergeStatePath = rootPath + ORI_PATH_MERGESTATE;
    MergeState state;
    string blob;
 ...
 * Clear merge state
 */
void LocalRepo::clearMergeState()
{
    string mergeStatePath = rootPath + ORI_PATH_MERGESTATE;
...
TempDir::sp LocalRepo::newTempDir()
{
...
string LocalRepo::getUUID()
{
    return id;
}

string LocalRepo::getVersion()
{
    return version;
}

string LocalRepo::getRootPath()
{
    return rootPath;
}

string LocalRepo::getLogPath()
{
    if (rootPath.compare("") == 0)
        return rootPath;
 ...
 * Peer Management
 */
map<string, Peer> LocalRepo::getPeers()
{
    return peers;
}

bool LocalRepo::addPeer(const string &name, const string &path)
{
...
bool LocalRepo::removePeer(const string &name)
{
    map<string, Peer>::iterator it = peers.find(name);
...
void LocalRepo::setInstaClone(const std::string &name, bool val)
{
    map<string, Peer>::iterator it = peers.find(name);
...
PrivateKey LocalRepo::getPrivateKey()
{
    PrivateKey key;
...
int publicKeyCB(map<string, PublicKey> *keys, const string &path)
{
...
map<string, PublicKey> LocalRepo::getPublicKeys()
{
...
string LocalRepo::findRootPath(const string &path)
{
    string root = path;
    if (path.size() == 0) {
