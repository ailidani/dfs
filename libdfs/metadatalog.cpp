...
void MetadataLog::open(const string &filename)
{
...
void MetadataLog::sync()
{
    ::fsync(fd);
}

void MetadataLog::rewrite(const RefcountMap *refs, const MetadataMap *data)
{
    if (refs == NULL)
        refs = &refcounts;
    if (data == NULL)
...
void MetadataLog::addRef(const ObjectHash &hash, MdTransaction::sp trs)
{
    if (!trs.get()) {
        trs = begin();
    }
...
refcount_t MetadataLog::getRefCount(const ObjectHash &hash) const
{
...
string MetadataLog::getMeta(const ObjectHash &hash, const string &key) const
{
...
MdTransaction::sp MetadataLog::begin()
{
    return MdTransaction::sp(new MdTransaction(this));
}

void MetadataLog::commit(MdTransaction *tr)
{
...
void MetadataLog::dumpRefs() const
{
...
void MetadataLog::dumpMeta() const
{
