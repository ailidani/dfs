...
bool HttpRepo::hasObject(const ObjectHash &id)
{
    ObjectHashVec vec;
    vector<bool> result;
...
vector<bool> HttpRepo::hasObjects(const ObjectHashVec &vec)
{
    strwstream ss;
    string resp;
    vector<bool> rval;
...
bytestream* HttpRepo::getObjects(const ObjectHashVec &vec)
{
    strwstream ss;
    ss.writeUInt32(vec.size());
    for (size_t i = 0; i < vec.size(); i++) {
...
std::set<ObjectInfo> HttpRepo::listObjects()
{
    set<ObjectInfo> rval;
...
int HttpRepo::addObject(ObjectType type, const ObjectHash &hash,
...
vector<Commit> HttpRepo::listCommits()
{
    vector<Commit> rval;
...
std::string& HttpRepo::_payload(const ObjectHash &id)
{
    return payloads[id];
}

void HttpRepo::_addPayload(const ObjectHash &id, const std::string &payload)
{
    payloads[id] = payload;
}

void HttpRepo::_clearPayload(const ObjectHash &id)
{
    payloads.erase(id);
}
