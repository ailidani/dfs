...
static void* ori_init(struct fuse_conn_info *conn)
{
...
static void ori_destroy(void *userdata)
{
...
static int ori_mknod(const char *path, mode_t mode, dev_t dev)
{
    return -EPERM;
}
...
static int ori_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    OriPriv *priv = GetOriPriv();
    string parentPath;
    OriDir *parentDir;
...
static int ori_open(const char *path, struct fuse_file_info *fi)
{
    OriPriv *priv = GetOriPriv();
    string parentPath;
    OriDir *parentDir;
...
static int ori_read(const char *path, char *buf, size_t size, off_t offset,
...
static void ori_setup_ori_oper()
{
