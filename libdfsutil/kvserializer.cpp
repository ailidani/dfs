...
void KVSerializer::putStr(const std::string &key, const std::string &value)
{
    table[key] = "S" + value;
}

void KVSerializer::putU8(const std::string &key, uint8_t value)
{
    string encoded = "BX";
...
void KVSerializer::putU16(const std::string &key, uint16_t value)
{
    string encoded = "WXX";
...
void KVSerializer::putU32(const std::string &key, uint32_t value)
{
...
void KVSerializer::putU64(const std::string &key, uint64_t value)
{
...
std::string KVSerializer::getStr(const std::string &key) const
{
...
uint8_t KVSerializer::getU8(const std::string &key) const
{
    const string encoded = table.at(key);
    uint8_t val;
...
uint16_t KVSerializer::getU16(const std::string &key) const
{
    const string encoded = table.at(key);
    uint16_t val;
...
uint32_t KVSerializer::getU32(const std::string &key) const
{
    const string encoded = table.at(key);
    uint32_t val;
...
uint64_t KVSerializer::getU64(const std::string &key) const
{
    const string encoded = table.at(key);
    uint64_t val;
...
KVSerializer::KVType KVSerializer::getType(const std::string &key) const
{
    const string encoded = table.at(key);
...
bool KVSerializer::hasKey(const std::string &key) const
{
    map<string, string>::const_iterator it = table.find(key);
    return it != table.end();
}

void KVSerializer::remove(const std::string &key)
{
    table.erase(key);
}

void KVSerializer::removeAll()
{
    table.clear();
}

void KVSerializer::fromBlob(const std::string &blob)
{
    int index = 0;
    int len = blob.length();
...
string KVSerializer::getBlob() const
{
    string blob;
