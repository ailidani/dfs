 ...
 */
void
dfs_log(int level, const char *fmt, ...)
{
    va_list ap;
    char buf[MAX_LOG];
    size_t off;
...
void ori_terminate() {
    ...
    if (!thrown) {
        try {
            ...
            dfs_log(LEVEL_SYS, "Caught unhandled exception: %s\n", e.what());
        } catch (...) {
            dfs_log(LEVEL_SYS, "Caught unhandled exception: (unknown type)");
        }
    }
    ...
    dfs_log(LEVEL_SYS, "Backtrace:\n");
    for (int i = 0; i < num; i++) {
        if (names != NULL)
            dfs_log(LEVEL_SYS, "[%d] %s\n", i, names[i]);
        else
            dfs_log(LEVEL_SYS, "[%d] [0x%p]\n", i, array[i]);
    }
    free(names);
#else
    dfs_log(LEVEL_SYS, "Backtrace not support not included in this build\n");
#endif /* HAVE_EXECINFO */
}
