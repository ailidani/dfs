...
void ObjectInfo::fromString(const std::string &info)
{
...
bool ObjectInfo::hasAllFields() const
{
    if (type == Null)
        return false;
    if (hash.isEmpty())
...
bool ObjectInfo::isCompressed() const
{
    return (flags & ORI_FLAG_ZIPMASK) == ORI_FLAG_UNCOMPRESSED;
}

ObjectInfo::ZipAlgo ObjectInfo::getAlgo() const {
...
void ObjectInfo::setAlgo(ObjectInfo::ZipAlgo algo)
{
    switch (algo) {
        case ZIPALGO_NONE:
