 ...
 *  libdfs
 *
 *  Created by Eddie Aili on 1/17/14.
 *  Copyright (c) 2014 Eddie Aili. All rights reserved.
 */
 ...
 */
bool OriFile_Exists(const string &path)
{
    struct stat sb;
...
int OriFile_MkDir(const string &path)
{
...
int OriFile_RmDir(const string &path)
{
 ...
 */
bool OriFile_IsDirectory(const string &path)
{
    struct stat sb;
 ...
 */
string OriFile_RealPath(const string &path)
{
    char *tmp;
    string rval = "";
 ...
 */
std::string OriFile_ReadFile(const string &path)
{
    diskstream ds(path);
    return ds.readAll();
 ...
 */
bool OriFile_WriteFile(const std::string &blob, const string &path)
{
 ...
 * Write an in memory blob to a file.
 */
bool OriFile_WriteFile(const char *blob, size_t len, const string &path)
{
    FILE *f;
    size_t bytesWritten;
 ...
 */
bool OriFile_Append(const std::string &blob, const string &path)
{
 ...
 * Append an in memory blob to a file.
 */
bool OriFile_Append(const char *blob, size_t len, const string &path)
{
    FILE *f;
    size_t bytesWritten;
 ...
 * Copy a file.
 */
int OriFile_Copy(const string &origPath, const string &newPath)
{
    int srcFd, dstFd;
 ...
 */
int OriFile_Move(const string &origPath, const string &newPath)
{
    int status = 0;
